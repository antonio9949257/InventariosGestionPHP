...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $con->prepare("SELECT id, nombre FROM categorias WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
      ...
      <div class="col-md-6">
        <div class="card bg-secondary text-light">
          <div class="card-header">
            <h2 class="card-title mb-0"><i class="fas fa-trash"></i> Eliminar Categoría</h2>
          </div>
          <div class="card-body">
            <?php echo $message; ?>
            <p>¿Estás seguro de que quieres eliminar la categoría <strong><?php echo htmlspecialchars($categoria['nombre']); ?></strong>?</p>
            <form action="delete_categoria.php?id=<?php echo $id; ?>" method="post">
